...
// require.main === module). It DOES call clip.read() once to seed lastHash.
...
// src/clipboard / src/server (which destructure execSync at load time).
...
// ── buildHTML ────────────────────────────────────────────────────────────────
describe("buildHTML", () => {
  const html = () => srv.buildHTML();

  test("returns a full HTML document", () => {
    const out = html();
    expect(typeof out).toBe("string");
    expect(out).toContain("<!DOCTYPE html>");
    expect(out).toContain("<title>Clip</title>");
  });

  test("contains the key UI element ids", () => {
    const out = html();
    expect(out).toContain('id="clipModal"');
    expect(out).toContain('id="searchModal"');
    expect(out).toContain('id="clipList"');
  });

  test("references the core client functions", () => {
    const out = html();
    expect(out).toContain("openModal");
    expect(out).toContain("dedupClips");
    expect(out).toContain("showQR");
  });

  test("links the manifest and icon", () => {
    const out = html();
    expect(out).toContain('href="/manifest.json"');
    expect(out).toContain('href="/icon.svg"');
  });
});
...
describe("poll", () => {
  ...
  test("adds new clipboard content to history", () => {
  ...
  test("is a no-op when the clipboard hash is unchanged since the last poll", () => {
    const text = u("repeat clipboard");
    bin.setClipboard(text);
    srv.poll();                 // records it, updates lastHash
    const after = db.count();
    srv.poll();                 // same hash → returns early
    expect(db.count()).toBe(after);
  });
});
...
describe("poll → peer sync", () => {
  ...
  test("sends new clipboard content to a connected peer", () => {
  ...
  test("records but does NOT send a clip larger than cfg.maxBytes to the peer", () => {
    const ws = connectedPeer();
...
describe("handlePeer", () => {
  ...
  test("writes a received clip to the clipboard and records it in history", () => {
    const ws = fakeWs();
    srv.handlePeer(ws);
  ...
  test("ignores non-'clip' message types", () => {
    const ws = fakeWs();
    srv.handlePeer(ws);
    const before = db.count();
  ...
  test("ignores a clip whose text exceeds cfg.maxBytes", () => {
    const ws = fakeWs();
    srv.handlePeer(ws);
  ...
  test("ignores a clip whose hash equals the current lastHash (echo guard)", () => {
...
describe("peer resilience", () => {
  ...
  test("handlePeer swallows a malformed (non-JSON) peer message without throwing", () => {
    const ws = new EventEmitter();
    ws.send = () => {};
    srv.handlePeer(ws);
    const before = db.count();
